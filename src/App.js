...
export class App extends Component {
  ...
  searchEvent=e=> {
    ...
    e.preventDefault()
    axios.get(`https://www.omdbapi.com/?apikey=2ff344e&s=${this.state.search}&plot=full`)
    .then(res=> res.data)
    ...
    .then(res=>{
      ...
      console.log(error)
      this.setState({setError:"Your fetching details are wrong please check details"})
    })
  }
  handleChange=e=> {
      this.setState({
  ...
  render() {
    ...
    return (
      ...
      { isLoading ? <Spinner /> :  <div className="Movies">
      {movieList.length > 0 ?(
        movieList.map(data=> (
